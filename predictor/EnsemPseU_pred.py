...
def kmer123(sequences):
    ...
    for seq in sequences:
        ...
        for j in range(len(seq)):
            tuple1[AADict[seq[j]]] = tuple1[AADict[seq[j]]] + 1
        if sum(tuple1) != 0:
            tuple1 = [i / sum(tuple1) for i in tuple1]

        tuple2 = [0] * 16
        for j in range(len(seq) - 2 + 1):
            tuple2[AADict[seq[j]] * 4 + AADict[seq[j + 1]]] = tuple2[AADict[seq[j]] * 4 + AADict[seq[j + 1]]] + 1
        if sum(tuple2) != 0:
            tuple2 = [i / sum(tuple2) for i in tuple2]

        tuple3 = [0] * 64
        for j in range(len(seq) - 3 + 1):
            tuple3[AADict[seq[j]] * 16 + AADict[seq[j + 1]] * 4 + AADict[seq[j + 2]]] = tuple3[AADict[seq[j]] * 16 + AADict[seq[j + 1]] * 4 + AADict[seq[j + 2]]] + 1
        if sum(tuple3) != 0:
            tuple3 = [i / sum(tuple3) for i in tuple3]

        tuple = tuple1+tuple2 + tuple3
        kmer123_feature.append(tuple)
...
def select_feature(feature_vector,type):
    if os.path.exists('selector_'+type) == False:
        s_feature_vector = feature_vector
    else:
        selector = []
        with open('selector_'+type) as fff:
            for line in fff:
